...
function loop() {
  ...
  snake.x += snake.dx;
  snake.y += snake.dy;
  // Если змейка достигла края поля по горизонтали — начинаем игру заново
  if (snake.x < 0) {
    snake.x = 160;
        snake.y = 160;
        snake.cells = [];
        ...
        bloc.x = getRandomInt(0, 25) * grid;
        bloc.y = getRandomInt(0, 25) * grid;
  }
  ...
  snake.cells.forEach(function (cell, index) {
    ...
    if (cell.x === apple.x && cell.y === apple.y) {
      ...
      apple.x = getRandomInt(0, 25) * grid;
      apple.y = getRandomInt(0, 25) * grid;
      bloc.x = getRandomInt(0, 24) * grid;
      bloc.y = getRandomInt(0, 24) * grid;
    }
